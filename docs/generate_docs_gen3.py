...
def format_resource_markdown(resource: Dict[str, Any], index: int) -> str:
    """
...
# --- Then optional cover image (per-resource) ---
    if cover_url:
        ...
        # Figure 1 caption (if we have any caption/type info)
        ...
        if caption_parts:
            md += f"*Figure {fig_counter}: " + " ".join(caption_parts) + "*\n\n"
        else:
           # no caption/type in YAML, still show a generic figure label
            md += f"*Figure {fig_counter}.*\n\n"
        ...
        if resources_for_topic:
            ...
            if len(resources_for_topic) > 1:
                ...
                for idx, res in enumerate(resources_for_topic, start=1):
                    ...
                    anchor = slugify(f"{idx}. {r_title}")
                    resources_list_md += f"{idx}. [{r_title}](#{anchor})\n"
                resources_list_md += "\n"

            # Then the actual resource blocks
            for idx, res in enumerate(resources_for_topic, start=1):
                resources_list_md += format_resource_markdown(res, idx)
        else:
